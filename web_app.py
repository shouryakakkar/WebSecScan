...
# Simple in-memory storage (will reset on each function invocation in serverless)
...
def scan():
    ...
    try:
        # Perform the scan
        logger.info(f"Starting web security scan for {target}")
        ...
        # Store results (will be lost on function restart in serverless)
...
def results(scan_id):
    ...
    if not scan_data:
        flash('Scan results not found or expired. Please run a new scan.', 'danger')
        return redirect(url_for('index'))
...
def download_pdf(scan_id):
    ...
    try:
        ...
        story.append(Paragraph(f"<b>Scan Time:</b> {scan_data.get('scan_time', 'N/A')}", styles['Normal']))
        story.append(Spacer(1, 12))
